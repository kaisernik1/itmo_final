    ...
    for _, f := range reader.File {
        if f.Name == "data.csv" {
            ...
            for _, row := range rows[1:] { // Пропускаем заголовок
                ...
                category := row[2]
                priceStr := row[3]
                createDateString := row[4]

                if idStr == "" || name == "" || category == "" || priceStr == "" || createDateString == "" {
...
func main() {
    ...
    router.HandleFunc("/api/v0/prices", handlePostRequest)
    router.HandleFunc("/api/v0/prices/download", handleGetRequest)
